...
abstract: |
  ...
  **Study Sample:** Cross-sectional audiometric data from the National Health and Nutrition Examination Survey (NHANES 2017–2020, n = 5,147), split into training (80%, n = 4,118) and held-out test (20%, n = 1,029) sets.

  **Results:** FAI showed strong agreement with WHO PTA-4 classification (weighted κ = 0.89). In borderline cases within ±5 dB of a severity boundary, FAI accuracy was 91.2% versus 68.4% for crisp PTA classification (p < 0.01). Configuration classification (flat, sloping, notched, precipitous, rising) achieved 89.4% agreement. At the 25/26 dB boundary, the system assigned graded membership (Normal μ = 0.40, Mild μ = 1.00) instead of a binary label.
...
We explored the NHANES audiometry dataset before building the model. The cohort (n = 5,147 participants, 3,539 ears with complete data after exclusions) had thresholds from −10 to 120 dB HL across all test frequencies.
...
Each category overlaps with its neighbours by about 5–10 dB at the classic WHO boundaries, which gives smooth transitions. A threshold of 26 dB HL, for example, gives membership μ = 0.40 to *Normal* and μ = 1.00 to *Mild*, reflecting its borderline status more honestly than a forced binary label.
...
A patient with a PTA of 24 dB is simultaneously 60% normal and 67% mild. The crisp label obscures this ambiguity. The FAI rises smoothly from 19.0 at 24 dB to 30.0 at 30 dB, unlike the step change at 26 dB that crisp rules impose.
...
**Case D (retiree, 72 years).** Asymmetric sensorineural loss. Right ear PTA-4 was 53.8 dB (Moderate) and left ear 28.8 dB (Mild). The asymmetry index of 30.0 dB placed this in the *Moderately Asymmetric* category, and the FAI adjusted the left ear rating up to 42.2 (Moderate) to reflect the clinical significance of the asymmetry. Here the fuzzy system captured a functional impact of asymmetry that ear-specific PTA classification overlooks.
...
Configuration classification reached 89.4% overall agreement against NHANES-derived labels (κ = 0.86). Per-type accuracy was highest for flat (94.2%) and sloping (91.5%), intermediate for precipitous (86.3%) and notched (84.1%), and lowest for rising (72.3%, small sample, n = 38). The main confusion was between gently sloping and steeply sloping patterns, which is not surprising given slope rate is continuous.
...
![Configuration classification confusion matrix. Overall accuracy: 89.4% (κ = 0.86). Primary confusion between gently sloping and steeply sloping patterns.](figures/fig6_confusion_matrix.png){#fig-5 width=80%}
...
**Figure 3.** (a) Bland-Altman plot showing agreement between FAI and PTA-4 reference on the held-out test set. (b) Classification accuracy as a function of distance from the nearest WHO severity boundary; FAI maintains >88% accuracy across all distances while PTA accuracy drops to 61% within ±3 dB.
...
(caption moved into Figure 3)

**Figure 5.** Configuration classification confusion matrix (heatmap). Overall accuracy 89.4%; primary confusion between gently sloping and steeply sloping patterns.
