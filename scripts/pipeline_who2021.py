...
def optimize_mfs(ear_rows):
    """Percentile-fit 7 trapezoidal MFs per frequency from training data.

    Categories with >= 10 observations per frequency are percentile-fitted
    (a=P5-2, b=P25, c=P75, d=P95+2, tails clipped); sparse categories use the
    WHO 2021 boundary-anchored default (:func:`_default_params`). Overlap
    between adjacent categories is then enforced to >= OVERLAP_MIN dB.
    """
    rows = []
    ...
    for freq in FREQUENCIES:
        ...
        for cat in SEVERITY_ORDER:
            ...
            if len(vals) < 10:
                trap[freq][cat] = _default_params(cat)
                continue
            p5, p25, p75, p95 = np.percentile(vals, [5, 25, 75, 95])
            a = max(0.0, p5 - 2); b = p25; c = p75
            d = 120.0 if cat == 'complete' else min(p95 + 2, 120.0)
    ...
    # Enforce >= OVERLAP_MIN positive overlap between adjacent categories
...
def main():
    ...
    opt = _minimize(_obj_kappa, np.array(DEFAULT_TH), method='Nelder-Mead',
                    options={'xatol': 0.5, 'fatol': 1e-6, 'maxiter': 800})
    label_th = np.sort(np.clip(opt.x, 5.0, 95.0)).tolist()
    gaps = np.diff(label_th)
    if gaps.min() < 2.0:
        print('  WARNING: calibrated thresholds degenerate; keeping WHO 2021 defaults')
