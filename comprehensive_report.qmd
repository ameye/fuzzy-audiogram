...
| Classification Agreement | Weighted κ = 0.76 |
| Overall Agreement (FAI vs WHO grade) | 91.4% |
| Clear-case Agreement | 95.2% |
| Borderline Agreement (±5 dB) | 74.9% (deliberate graded reclassification) |
| Configuration Classification | 6 canonical shapes (slope-based) |
| Spearman ρ vs. PTA-4 | 0.61* |
...
> **Note:** the membership functions were re-optimised on the combined training set for this analysis (15,654 ears from the three adult cycles; see Methodology). The FIS structure is cycle-independent; the parameters shipped in the deployed web application are the earlier 2017–2020 fit and are disclosed separately.

## Optimised Parameters

**Re-optimised on the combined training set (aggregated across 7 frequencies, exactly 2.0 dB overlap at every boundary):**
...
|------|-------|-----------|-----|---------------|-------------|
| A (Textile worker, 58y) | 27.5 dB | Mild | 20.4 | Precipitous (shape rules) | Borderline membership (Normal 0.04, Mild 1.00) |
| B (Military officer, 34y) | 23.0 dB | Normal | 17.0 | Precipitous (shape rules) | 4 kHz notch (25 dB) masked by PTA |
...
**Case A** — PTA-4 of 27.5 dB is classified as "Mild" by WHO criteria; the FIS returns FAI = 20.4 (Mild), with Normal μ = 0.04 and Mild μ = 1.00 — a continuous score that sits just above the fuzzy Normal/Mild threshold, reflecting the borderline status.
...
- **κ = 0.76** agreement with WHO PTA-4 classification (substantial)
- **95.2% agreement in clear cases** — matching the crisp reference where classification is uncontroversial
- **Graded reclassification in the borderline zone** — 74.9% agreement with the crisp label within ±5 dB (a zone containing 18.8% of test ears), reflecting deliberate, continuous membership (e.g., 27 dB → Normal μ 0.07 / Mild μ 0.25) instead of a forced binary choice
