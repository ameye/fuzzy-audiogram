...
abstract: |
  ...
  **Methods:** We extracted audiometric data from three National Health and Nutrition Examination Survey (NHANES) cycles that tested adults aged 20–69 y (1999–2000, 2011–2012, 2015–2016; n = 10,889) and split them by participant into training (80%, n = 15,656 ears) and held-out test (20%, n = 3,912 ears). We used overlapping trapezoidal membership functions at each test frequency (0.5–8 kHz), optimised on the training distribution, with a rule base of 48 audiology-derived Mamdani-type fuzzy rules. We defuzzified outputs via centroid method into a continuous Fuzzy Audiometric Index (FAI, 0–100). Comparator models included standard PTA-4 WHO classification, XGBoost, and Random Forest.

  **Results:** On the held-out test set, the FAI showed substantial classification agreement (weighted κ = 0.93) and 98.1% agreement in clear cases. In borderline cases (±5 dB of a severity boundary) — 18.7% of test ears — the system returned graded membership rather than a forced binary label (79.8% agreement with the crisp grade, reflecting the designed reclassification of ambiguous thresholds). Configuration typing is provided as a graded vector over six canonical shapes. At the normal–mild transition, membership grades continuously (e.g., 26 dB: Normal μ = 0.16, Mild μ = 0.40) instead of flipping at 26 dB.
...
![Borderline case analysis: FAI agreement with the crisp WHO grade as a function of distance from the nearest severity boundary on the held-out test set (98.1% clear-case, 79.8% within ±5 dB).](figures/fig5_borderline_analysis.png){width=80%}
...
**Figure 5.** Borderline case analysis: FAI agreement with the crisp WHO grade as a function of distance from the nearest boundary on the held-out test set — 98.1% in clear cases (≥6 dB from a boundary) and 79.8% within ±5 dB, with agreement recovering monotonically as distance increases.
