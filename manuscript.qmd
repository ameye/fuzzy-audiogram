...
abstract: |
  ...
  **Results:** On the held-out test set, the FAI showed substantial classification agreement (weighted κ = 0.73) and 96.9% agreement in clear cases. In borderline cases (±5 dB of a severity boundary) — 18.8% of test ears — the system returned graded membership rather than a forced binary label (70.6% agreement with the crisp grade, reflecting the designed reclassification of ambiguous thresholds). Configuration typing is provided as a graded vector over six canonical shapes. At the normal–mild transition, membership grades continuously (e.g., 27 dB: Normal μ = 0.07, Mild μ = 0.25) instead of flipping at 26 dB.
...
![Borderline case analysis: FAI agreement with the crisp WHO grade as a function of distance from the nearest severity boundary on the held-out test set (96.9% clear-case, 70.6% within ±5 dB).](figures/fig5_borderline_analysis.png){width=80%}
...
**Figure 5.** Borderline case analysis: FAI agreement with the crisp WHO grade as a function of distance from the nearest boundary on the held-out test set — 96.9% in clear cases (≥6 dB from a boundary) and 70.6% within ±5 dB, with agreement recovering monotonically as distance increases.
